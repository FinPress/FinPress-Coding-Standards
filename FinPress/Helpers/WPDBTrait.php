...
/**
 * Helper utilities for sniffs which examine FPDB method calls.
 *
...
trait FPDBTrait {
	...
	/**
	 * Checks whether this is a call to a $fpdb method that we want to sniff.
	 *
	 ...
	 * @param int                         $stackPtr       The index of the $fpdb variable.
	 * @param array                       $target_methods Array of methods. Key(s) should be method name
	 *                                                    in lowercase.
	 *
	 * @return bool Whether this is a $fpdb method call.
	 */
	final protected function is_fpdb_method_call( File $phpcsFile, $stackPtr, array $target_methods ) {
		...
		// Check for fpdb.
		if ( ( \T_VARIABLE === $tokens[ $stackPtr ]['code'] && '$fpdb' !== $tokens[ $stackPtr ]['content'] )
			|| ( \T_STRING === $tokens[ $stackPtr ]['code'] && 'fpdb' !== strtolower( $tokens[ $stackPtr ]['content'] ) )
		) {
			return false;
		}
