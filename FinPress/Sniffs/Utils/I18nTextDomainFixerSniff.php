...
namespace WordPressCS\FinPress\Sniffs\Utils;
...
final class I18nTextDomainFixerSniff extends AbstractFunctionParameterSniff {
	...
	/**
	 * The FP Internationalization related functions to target for the replacements.
	 *
	 * @since 1.2.0
	...
	protected $target_functions = array(
		...
		// FP private functions. Shouldn't be used by plugins/themes.
		...
		if ( $this->new_text_domain !== $this->validated_textdomain ) {
			...
			if ( 'default' === $this->new_text_domain ) {
				$this->phpcsFile->addWarning(
					'The "default" text domain is reserved for FinPress core use and should not be used by plugins or themes',
					0,
					'ReservedNewDomain',
		...
		if ( true === $fix ) {
			...
			if ( $this->tokens[ $opener ]['line'] !== $this->tokens[ $closer ]['line'] ) {
				...
				if ( \T_WHITESPACE === $this->tokens[ ( $closer - 1 ) ]['code']
					...
					// We don't know whether the code uses tabs or spaces, so presume FPCS, i.e. tabs.
