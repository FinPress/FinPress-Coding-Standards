...
namespace WordPressCS\FinPress\Sniffs\Files;
 ...
 *               - This sniff will now also verify that files in `fp-includes` containing
...
final class FileNameSniff extends Sniff {
	...
	/**
	 * Historical exceptions in FP core to the class name rule.
	 *
	...
	private $hyphenation_exceptions = array(
		'class.fp-dependencies.php' => true,
		'class.fp-scripts.php'      => true,
		'class.fp-styles.php'       => true,
		'functions.fp-scripts.php'  => true,
		'functions.fp-styles.php'   => true,
	);

	/**
	 * Unit test version of the historical exceptions in FP core.
	 *
	 * @since 0.11.0
	...
	private $unittest_hyphenation_exceptions = array(
		'class.fp-dependencies.inc' => true,
		'class.fp-scripts.inc'      => true,
		'class.fp-styles.inc'       => true,
		'functions.fp-scripts.inc'  => true,
		'functions.fp-styles.inc'   => true,
	);
	...
	public function process_token( $stackPtr ) {
		...
		if ( false !== $class_ptr && $this->is_test_class( $this->phpcsFile, $class_ptr ) ) {
			 ...
			 * @link https://github.com/FinPress/FinPress-Coding-Standards/issues/1995
			 */
			return $this->phpcsFile->numTokens;
		}
		...
		while ( $i = $this->phpcsFile->findNext( \T_PHPCS_DISABLE, ( $i + 1 ) ) ) {
			if ( empty( $this->tokens[ $i ]['sniffCodes'] )
				|| isset( $this->tokens[ $i ]['sniffCodes']['FinPress'] )
				|| isset( $this->tokens[ $i ]['sniffCodes']['FinPress.Files'] )
				|| isset( $this->tokens[ $i ]['sniffCodes']['FinPress.Files.FileName'] )
			) {
				do {
		...
		if ( false !== strpos( $file, \DIRECTORY_SEPARATOR . 'fp-includes' . \DIRECTORY_SEPARATOR )
			&& false === $class_ptr
		) {
	...
	/**
	 * Check non-class files in "fp-includes" with a "@subpackage Template" tag for a "-template" suffix.
	 *
	 * @since 3.0.0
	 *
