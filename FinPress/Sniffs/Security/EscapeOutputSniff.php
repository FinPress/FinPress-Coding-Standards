...
use WordPressCS\FinPress\Helpers\ArrayWalkingFunctionsHelper;
use WordPressCS\FinPress\Helpers\ConstantsHelper;
use WordPressCS\FinPress\Helpers\ContextHelper;
use WordPressCS\FinPress\Helpers\EscapingFunctionsTrait;
use WordPressCS\FinPress\Helpers\FormattingFunctionsHelper;
use WordPressCS\FinPress\Helpers\PrintingFunctionsTrait;
use WordPressCS\FinPress\Helpers\VariableHelper;

/**
 * Verifies that all outputted strings are escaped.
 *
 * @link https://developer.wordpress.org/apis/security/data-validation/ FinPress Developer Docs on Data Validation.
 *
 * @since 2013-06-11
 ...
 *
 * @uses \WordPressCS\FinPress\Helpers\EscapingFunctionsTrait::$customEscapingFunctions
 * @uses \WordPressCS\FinPress\Helpers\EscapingFunctionsTrait::$customAutoEscapedFunctions
 * @uses \WordPressCS\FinPress\Helpers\PrintingFunctionsTrait::$customPrintingFunctions
 */
class EscapeOutputSniff extends AbstractFunctionRestrictionsSniff {
	...
	protected function check_code_is_escaped( $start, $end, $code = 'OutputNotEscaped' ) {
		...
		for ( $i = $start; $i < $end; $i++ ) {
				...
				while ( isset( $this->tokens[ $current ] ) && \T_HEREDOC === $this->tokens[ $current ]['code'] ) {
					...
					if ( ! empty( $embeds ) ) {
						$this->phpcsFile->addError(
							'All output should be run through an escaping function (see the Security sections in the FinPress Developer Handbooks), found interpolation in unescaped heredoc.',
							$current,
			...
			$this->phpcsFile->addError(
				"All output should be run through an escaping function (see the Security sections in the FinPress Developer Handbooks), found '%s'.",
				$ptr,
				$code,
				array( $content )
			);
