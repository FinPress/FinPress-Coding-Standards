...
use WordPressCS\FinPress\Helpers\SanitizationHelperTrait;
use WordPressCS\FinPress\Helpers\ValidationHelper;
 ...
 *
 * @link https://github.com/FinPress/FinPress-Coding-Standards/issues/69
 *
 * @since 0.3.0
 ...
 *
 * @uses \WordPressCS\FinPress\Helpers\SanitizationHelperTrait::$customSanitizingFunctions
 * @uses \WordPressCS\FinPress\Helpers\SanitizationHelperTrait::$customUnslashingSanitizingFunctions
 */
class ValidatedSanitizedInputSniff extends Sniff {
	...
	/**
	 * Superglobals for which the values will be slashed by FP.
	 *
	 * @link https://developer.wordpress.org/reference/functions/fp_magic_quotes/
	 *
	 * @since 3.0.0
	 *
	...
	public function add_unslash_error( File $phpcsFile, $stackPtr ) {
		...
		if ( isset( $this->slashed_superglobals[ $var_name ] ) === false ) {
			// FP doesn't slash these, so they don't need unslashing.
			return;
		}
		...
		$phpcsFile->addError(
			'%s not unslashed before sanitization. Use fp_unslash() or similar',
			$stackPtr,
			'MissingUnslash',
			$error_data
		);
