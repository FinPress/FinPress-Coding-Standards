...
use WordPressCS\FinPress\Helpers\ContextHelper;
...
/**
 * Warns about overwriting FinPress native global variables.
 *
 * @since 0.3.0
 * @since 0.4.0  This class now extends the WordPressCS native `Sniff` class.
 * @since 0.12.0 The $fp_globals property has been moved to the `Sniff` class.
 * @since 0.13.0 Class name changed: this class is now namespaced.
 * @since 1.0.0  This sniff has been moved from the `Variables` category to the `FP`
...
final class GlobalVariablesOverrideSniff extends Sniff {
	...
	/**
	 * Allow select variables from the FPGlobalVariablesHelper::$fp_globals array to be overwritten.
	 *
	 * A few select variables in FP Core are _intended_ to be overwritten
	...
	protected function process_variable_assignment( $stackPtr, $in_list = false ) {
		...
		/*
		 * Is this one of the FP global variables ?
		 */
		if ( FPGlobalVariablesHelper::is_fp_global( $var_name ) === false ) {
			return;
		}

		/*
		 * Is this one of the FP global variables which are allowed to be overwritten ?
		 */
		...
		/*
		 * Function parameters with the same name as a FP global variable are fine,
		...
		/*
		 * Class property declarations with the same name as FP global variables are fine.
		 */
		if ( false === $in_list && true === Scopes::isOOProperty( $this->phpcsFile, $stackPtr ) ) {
			return;
		}

		// Still here ? In that case, the FP global variable is being tampered with.
	...
	protected function process_global_statement( $stackPtr, $in_function_scope ) {
		...
		while ( isset( $this->tokens[ $ptr ] ) && $ptr < $end_of_statement ) {
			if ( \T_VARIABLE === $this->tokens[ $ptr ]['code'] ) {
				...
				if ( FPGlobalVariablesHelper::is_fp_global( $var_name )
	...
	protected function add_error( $stackPtr, $data = array() ) {
		...
		$this->phpcsFile->addError(
			'Overriding FinPress globals is prohibited. Found assignment to %s',
			$stackPtr,
			'Prohibited',
			$data
		);
