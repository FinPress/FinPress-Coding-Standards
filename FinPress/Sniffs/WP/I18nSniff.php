...
/**
 * Makes sure FP internationalization functions are used properly.
 *
...
final class I18nSniff extends AbstractFunctionParameterSniff {
	...
	/**
	 * The I18N functions in use in FP.
	 *
	 * @since 0.10.0
	...
	private function check_argument_is_string_literal( $stackPtr, $matched_content, $param_name, $param_info ) {
		...
		if ( false === $param_info || '' === $param_info['clean'] ) {
			 ...
			 * when set to `default` (= FP Core translation).
			 */
			if ( 'domain' === $param_name ) {
				if ( empty( $this->text_domain ) ) {
					// If no text domain is passed, presume FP Core.
					return false;
				}
				...
				if ( true === $this->text_domain_contains_default ) {
					$this->phpcsFile->addWarning(
						'Missing $%s parameter in function call to %s(). If this text string is supposed to use a FP Core translation, use the "default" text domain.',
						$stackPtr,
						$error_code . 'Default',
						array( $param_name, $matched_content )
					);
