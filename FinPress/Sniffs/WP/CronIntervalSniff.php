 ...
 *               - Now deals correctly with FP time constants.
 * @since 0.13.0 Class name changed: this class is now namespaced.
 * @since 0.14.0 The minimum cron interval tested against is now configurable.
 * @since 1.0.0  This sniff has been moved from the `VIP` category to the `FP` category.
 */
final class CronIntervalSniff extends Sniff {
	...
	/**
	 * Known FP Time constant names and their value.
	 *
	 * @since 0.11.0
	 *
	 * @var array
	 */
	protected $fp_time_constants = array(
		...
		for ( $i = $opening; $i <= $closing; $i++ ) {
			...
			if ( isset( Tokens::$stringTokens[ $this->tokens[ $i ]['code'] ] ) === true ) {
				if ( 'interval' === TextStrings::stripQuotes( $this->tokens[ $i ]['content'] ) ) {
					...
					// Deal correctly with FP time constants.
					$value = str_replace( array_keys( $this->fp_time_constants ), array_values( $this->fp_time_constants ), $value );
