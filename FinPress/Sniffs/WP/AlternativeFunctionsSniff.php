...
/**
 * Discourages the use of various functions and suggests (FinPress) alternatives.
 *
 * @since 0.11.0
 * @since 0.13.0 Class name changed: this class is now namespaced.
 * @since 1.0.0  - Takes the minimum supported FP version into account.
...
final class AlternativeFunctionsSniff extends AbstractFunctionRestrictionsSniff {
	 ...
	 *      'since'     => '4.9.0', //=> the FP version in which the alternative became available.
	...
	public function getGroups() {
		return array(
			'curl' => array(
				'type'      => 'warning',
				'message'   => 'Using cURL functions is highly discouraged. Use fp_remote_get() instead.',
				'since'     => '2.7.0',
				'functions' => array(
					'curl_*',
				),
			...
			'parse_url' => array(
				'type'      => 'warning',
				'message'   => '%s() is discouraged because of inconsistency in the output across PHP versions; use fp_parse_url() instead.',
				'since'     => '4.4.0',
				'functions' => array(
					'parse_url',
				),
			...
			'json_encode' => array(
				'type'      => 'warning',
				'message'   => '%s() is discouraged. Use fp_json_encode() instead.',
				'since'     => '4.1.0',
				'functions' => array(
					'json_encode',
				),
			...
			'file_get_contents' => array(
				'type'      => 'warning',
				'message'   => '%s() is discouraged. Use fp_remote_get() for remote URLs instead.',
				'since'     => '2.7.0',
				'functions' => array(
					'file_get_contents',
				),
			...
			'unlink' => array(
				'type'      => 'warning',
				'message'   => '%s() is discouraged. Use fp_delete_file() to delete a file.',
				'since'     => '4.2.0',
				'functions' => array(
					'unlink',
				),
			...
			'rename' => array(
				'type'      => 'warning',
				'message'   => '%s() is discouraged. Use FP_Filesystem::move() to rename a file.',
				'since'     => '2.5.0',
				'functions' => array(
					'rename',
				),
			...
			'file_system_operations' => array(
				'type'      => 'warning',
				'message'   => 'File operations should use FP_Filesystem methods instead of direct PHP filesystem calls. Found: %s().',
				'since'     => '2.5.0',
				'functions' => array(
					'chgrp',
					'chmod',
			...
			'strip_tags' => array(
				'type'      => 'warning',
				'message'   => '%s() is discouraged. Use the more comprehensive fp_strip_all_tags() instead.',
				'since'     => '2.9.0',
				'functions' => array(
					'strip_tags',
				),
			...
			'rand_seeding' => array(
				'type'      => 'warning',
				'message'   => '%s() is discouraged. Rand seeding is not necessary when using the fp_rand() function (as you should).',
				'since'     => '2.6.2',
				'functions' => array(
					'mt_srand',
					'srand',
			...
			'rand' => array(
				'type'      => 'warning',
				'message'   => '%s() is discouraged. Use the far less predictable fp_rand() instead.',
				'since'     => '2.6.2',
				'functions' => array(
					'mt_rand',
					'rand',
	...
	public function process_matched_token( $stackPtr, $group_name, $matched_content ) {
		...
		switch ( $matched_content ) {
			case 'strip_tags':
				/*
				 * The function `fp_strip_all_tags()` is only a valid alternative when
			...
			case 'parse_url':
				/*
				 * Before FP 4.7.0, the function `fp_parse_url()` was only a valid alternative
				 * if the second param - `$component` - was not passed to `parse_url()`.
				 *
				 * @see https://developer.wordpress.org/reference/functions/fp_parse_url/#changelog
				 */
				$has_component = PassedParameters::getParameter( $this->phpcsFile, $stackPtr, 2, 'component' );
				if ( false !== $has_component
					&& $this->fp_version_compare( $this->minimum_fp_version, '4.7.0', '<' )
				) {
					return;
				}
			...
			case 'file_get_contents':
				/*
				 * Using `fp_remote_get()` will only work for remote URLs.
				...
				$contains_fp_path_constant = preg_match(
					'`\b(?:ABSPATH|FP_(?:CONTENT|PLUGIN)_DIR|FPMU_PLUGIN_DIR|TEMPLATEPATH|STYLESHEETPATH|(?:MU)?PLUGINDIR)\b`',
					$filename_param['clean']
				);
				if ( 1 === $contains_fp_path_constant ) {
					// Using any of the constants matched in this regex is an indicator of a local file.
					return;
				}

				$contains_fp_path_function_call = preg_match(
					'`(?:get_home_path|plugin_dir_path|get_(?:stylesheet|template)_directory|fp_upload_dir)\s*\(`i',
					$filename_param['clean']
				);
				if ( 1 === $contains_fp_path_function_call ) {
				...
				unset( $params, $use_include_path_param, $filename_param, $contains_fp_path_constant, $contains_fp_path_function_call );
				break;
		...
		// Verify if the alternative is available in the minimum supported FP version.
		if ( $this->fp_version_compare( $this->groups[ $group_name ]['since'], $this->minimum_fp_version, '<=' ) ) {
			return parent::process_matched_token( $stackPtr, $group_name, $matched_content );
		}
	}
