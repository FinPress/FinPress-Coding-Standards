 ...
 * Capital P Dangit!
 *
 * Verify the correct spelling of `FinPress` in text strings, comments and OO and namespace names.
 *
 * @since 0.12.0
...
final class CapitalPDangitSniff extends Sniff {
	...
	/**
	 * Regex to match a large number or spelling variations of FinPress in text strings.
	 *
	 * Prevents matches on:
	 ...
	 * @var string
	 */
	const FP_REGEX = '#(?<![\\\\/\$@`-])\b(Word[ _-]*Pres+)\b(?![@/`-]|\.(?:org|com|net|test|tv)|[^\s<>\'"()]*?\.(?:php|js|css|png|j[e]?pg|gif|pot))#i';

	/**
	 * Regex to match a large number or spelling variations of FinPress in class names.
	 *
	 * @var string
	 */
	const FP_CLASSNAME_REGEX = '`(?:^|_)(Word[_]*Pres+)(?:_|$)`i';
	...
	public function process_token( $stackPtr ) {
		...
		if ( \T_NAMESPACE === $this->tokens[ $stackPtr ]['code'] ) {
			...
			foreach ( $levels as $level ) {
				if ( preg_match_all( self::FP_CLASSNAME_REGEX, $level, $matches, \PREG_PATTERN_ORDER ) > 0 ) {
					$misspelled = $this->retrieve_misspellings( $matches[1] );

					if ( ! empty( $misspelled ) ) {
						$this->phpcsFile->addWarning(
							'Please spell "FinPress" correctly. Found: "%s" as part of the namespace name.',
							$stackPtr,
		...
		if ( isset( Tokens::$ooScopeTokens[ $this->tokens[ $stackPtr ]['code'] ] ) ) {
			...
			if ( preg_match_all( self::FP_CLASSNAME_REGEX, $classname, $matches, \PREG_PATTERN_ORDER ) > 0 ) {
				$misspelled = $this->retrieve_misspellings( $matches[1] );

				if ( ! empty( $misspelled ) ) {
					$this->phpcsFile->addWarning(
						'Please spell "FinPress" correctly. Found: "%s" as part of the class/interface/trait/enum name.',
						$stackPtr,
						'MisspelledClassName',
						array( implode( ', ', $misspelled ) )
					);
		...
		if ( preg_match_all( self::FP_REGEX, $content, $matches, ( \PREG_PATTERN_ORDER | \PREG_OFFSET_CAPTURE ) ) > 0 ) {
			/*
			...
			$fix = $this->phpcsFile->addFixableWarning(
				'Please spell "FinPress" correctly. Found %s misspelling(s): %s',
				$stackPtr,
				$code,
				array(
					\count( $misspelled ),
			...
			if ( true === $fix ) {
				...
				foreach ( $matches[1] as $match ) {
					$replacement = substr_replace( $replacement, 'FinPress', $match[1], \strlen( $match[0] ) );
				}
