...
final class CapabilitiesSniff extends AbstractFunctionParameterSniff {
	 ...
	 *
	 * The functions are defined in `fp-admin/includes/plugin.php` and
	 * `/fp-includes/capabilities.php`.
	...
	protected $target_functions = array(
		...
		'add_object_page'           => array( // Deprecated since FP 4.5.0.
			'position' => 3,
			'name'     => 'capability',
		),
		...
		'add_utility_page'          => array( // Deprecated since FP 4.5.0.
			'position' => 3,
			'name'     => 'capability',
		),
		'author_can'                => array(
	 ...
	 * @since 3.0.0
	 *
	 * @var array<string, true> Key is role available in FP Core, value irrelevant.
	 */
	 ...
	 * - The `map_meta_cap()` function in the `src/fp-includes/capabilities.php` file.
	 ...
	 *
	 * {@internal To be updated after every major release. Last updated for FinPress 6.1.0.}
	 *
	 * @link https://github.com/FinPress/wordpress-develop/blob/master/tests/phpunit/tests/user/capabilities.php
	 *
	 * @since 3.0.0
	 *
	...
	public function process_parameters( $stackPtr, $group_name, $matched_content, $parameters ) {
		...
		if ( isset( $this->deprecated_capabilities[ $matched_parameter ] ) ) {
			...
			$is_error = $this->fp_version_compare( $this->deprecated_capabilities[ $matched_parameter ], $this->minimum_fp_version, '<' );
			...
			MessageHelper::addMessage(
				$this->phpcsFile,
				'The capability "%s", found in the function call to %s(), has been deprecated since FinPress version %s.',
				$first_non_empty,
				$is_error,
				'Deprecated',
				$data
		...
		$this->phpcsFile->addWarning(
			'Found unknown capability "%s" in function call to %s(). Please check the spelling of the capability. If this is a custom capability, please verify the capability is registered with FinPress via a call to FP_Role(s)->add_cap().' . \PHP_EOL . 'Custom capabilities can be made known to this sniff by setting the "custom_capabilities" property in the PHPCS ruleset.',
			$first_non_empty,
			'Unknown',
			array(
				$matched_parameter,
