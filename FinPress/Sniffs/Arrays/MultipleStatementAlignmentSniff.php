...
namespace WordPressCS\FinPress\Sniffs\Arrays;
...
final class MultipleStatementAlignmentSniff extends Sniff {
	 ...
	 *
	 * While the FP standard does not allow single line multi-item associative arrays,
	 * this sniff should function independently of that.
	 *
	 * The `FinPress.WhiteSpace.OperatorSpacing` sniff already covers checking that
	...
	protected function process_multi_line_array( $stackPtr, $items, $opener, $closer ) {
		 ...
		 * That's for the `FinPress.Arrays.ArrayIndentation` sniff to fix.
	...
	protected function validate_align_multiline_items() {
		...
		$this->phpcsFile->addError(
			'Invalid property value passed: "%s". The value for the "alignMultilineItems" property for the "FinPress.Arrays.MultipleStatementAlignment" sniff should be either "always", "never" or an comparison operator + a number between 0 and 100.',
			0,
