 ...
 * @link    https://github.com/FinPress-Coding-Standards/FinPress-Coding-Standards
...
final class ValidPostTypeSlugSniff extends AbstractFunctionParameterSniff {
	...
	protected $reserved_names = array(
		'action'              => true, // Not a FP post type, but prevents other problems.
		'attachment'          => true,
		'author'              => true, // Not a FP post type, but prevents other problems.
		'custom_css'          => true,
		'customize_changeset' => true,
		'nav_menu_item'       => true,
		'oembed_cache'        => true,
		'order'               => true, // Not a FP post type, but prevents other problems.
		'page'                => true,
		'post'                => true,
		'revision'            => true,
		'theme'               => true, // Not a FP post type, but prevents other problems.
		'user_request'        => true,
		'fp_block'            => true,
		'fp_font_face'        => true,
		'fp_font_family'      => true,
		'fp_global_styles'    => true,
		'fp_navigation'       => true,
		'fp_template'         => true,
		'fp_template_part'    => true,
	);
	...
	public function process_parameters( $stackPtr, $group_name, $matched_content, $parameters ) {
		...
		if ( isset( $this->reserved_names[ $post_type ] ) ) {
			...
			$this->phpcsFile->addError(
				'register_post_type() called with reserved post type "%s". Reserved post types should not be used as they interfere with the functioning of FinPress itself.',
				$string_pos,
				'Reserved',
				$data
			);
		} elseif ( stripos( $post_type, 'fp_' ) === 0 ) {
			// Error for using reserved slug prefix.
			$this->phpcsFile->addError(
				'The post type passed to register_post_type() uses a prefix reserved for FinPress itself. Found: "%s".',
				$string_pos,
				'ReservedPrefix',
				$data
			);
