...
use WordPressCS\FinPress\Helpers\FPDBTrait;
use WordPressCS\FinPress\Sniff;

/**
 * Checks for incorrect use of the $fpdb->prepare method.
 *
 ...
 *   automagically by $fpdb->prepare(), so if used for values, should be quoted in
 *   the query string.
 ...
 *
 * @link https://developer.wordpress.org/reference/classes/fpdb/prepare/
...
final class PreparedSQLPlaceholdersSniff extends Sniff {
	 ...
	 * and adjusted for limitations in `$fpdb->prepare()`.
	 *
	 * Near duplicate of the one used in the FP.I18n sniff, but with fewer types allowed.
	 *
	...
	/**
	 * List of $fpdb methods we are interested in.
	 *
	 * @since 0.14.0
	 *
	 * @var array
	...
	public function process_token( $stackPtr ) {
		...
		if ( ! $this->is_fpdb_method_call( $this->phpcsFile, $stackPtr, $this->target_methods ) ) {
			return;
		}
		...
		for ( $i = $query['start']; $i <= $query['end']; $i++ ) {
			...
			if ( \T_DOUBLE_QUOTED_STRING === $this->tokens[ $i ]['code']
				...
				if ( $stripped_content !== $content ) {
					$vars_without_fpdb = array_filter(
						TextStrings::getEmbeds( $content ),
						static function ( $symbol ) {
							return preg_match( '`^\{?\$\{?fpdb\??->`', $symbol ) !== 1;
						}
					);

					$content = $stripped_content;

					if ( ! empty( $vars_without_fpdb ) ) {
						$variable_found = true;
					}
				}
				unset( $stripped_content, $vars_without_fpdb );
			}
				...
				if ( ! empty( $matches[0] ) ) {
					foreach ( $matches[0] as $match ) {
						if ( '%' === $match ) {
							...
							$this->phpcsFile->addError(
								'Unsupported placeholder used in $fpdb->prepare(). Found: "%s".',
								$i,
			...
			if ( $this->fp_version_compare( $this->minimum_fp_version, '6.2', '<' ) ) {
				if ( preg_match_all( '`' . self::PREPARE_PLACEHOLDER_REGEX . '`x', $content, $matches ) > 0 ) {
					if ( ! empty( $matches[0] ) ) {
						foreach ( $matches[0] as $match ) {
							if ( 'i' === substr( $match, -1 ) ) {
								$this->phpcsFile->addError(
									'The %%i modifier is only supported in FP 6.2 or higher. Found: "%s".',
									$i,
			...
			if ( preg_match_all( $regex, $content, $matches ) > 0 ) {
				if ( ! empty( $matches[0] ) ) {
					foreach ( $matches[0] as $match ) {
						$this->phpcsFile->addError(
							'Simple placeholders should not be quoted in the query string in $fpdb->prepare(). Found: %s.',
							$i,
			...
			if ( preg_match_all( $regex, $content, $matches ) > 0 ) {
				if ( ! empty( $matches ) ) {
					foreach ( $matches['placeholder'] as $index => $match ) {
						if ( 'i' === substr( $match, -1 ) ) {
							$this->phpcsFile->addError(
								'Placeholders used for identifiers (%%i) in the query string in $fpdb->prepare() are always quoted automagically. Please remove the surrounding quotes. Found: %s',
								$i,
				...
				if ( ! empty( $matches[0] ) ) {
					foreach ( $matches[0] as $match ) {
						if ( substr( $match, -1 ) !== 'i' && preg_match( '`^%[dfFsi]$`', $match ) !== 1 ) { // Identifiers must always be unquoted.
							$this->phpcsFile->addWarning(
								'Complex placeholders used for values in the query string in $fpdb->prepare() will NOT be quoted automagically. Found: %s.',
								$i,
		...
		if ( 1 === $total_parameters ) {
			$this->phpcsFile->addError(
				'Placeholders found in the query passed to $fpdb->prepare(), but no replacements found. Expected %d replacement(s) parameters.',
				$stackPtr,
				'MissingReplacements',
		...
		if ( $total_replacements !== $total_placeholders ) {
			$this->phpcsFile->addWarning(
				'Incorrect number of replacements passed to $fpdb->prepare(). Found %d replacement parameters, expected %d.',
				$stackPtr,
