...
final class PreparedSQLSniff extends Sniff {
	...
	/**
	 * Tokens that we don't flag when they are found in a $fpdb method call.
	 *
	...
	public function process_token( $stackPtr ) {
		...
		if ( ! $this->is_fpdb_method_call( $this->phpcsFile, $stackPtr, $this->methods ) ) {
			return;
		}
		...
		for ( $this->i; $this->i < $this->end; $this->i++ ) {
			...
			if ( \T_DOUBLE_QUOTED_STRING === $this->tokens[ $this->i ]['code']
				...
				foreach ( $bad_variables as $bad_variable ) {
					$this->phpcsFile->addError(
						'Use placeholders and $fpdb->prepare(); found interpolated variable %s at %s',
						$this->i,
			...
			if ( \T_VARIABLE === $this->tokens[ $this->i ]['code'] ) {
				if ( '$fpdb' === $this->tokens[ $this->i ]['content'] ) {
					$this->is_fpdb_method_call( $this->phpcsFile, $this->i, $this->methods );
					continue;
				}
			...
			$this->phpcsFile->addError(
				'Use placeholders and $fpdb->prepare(); found %s',
				$this->i,
				'NotPrepared',
