 ...
 * @since 3.0.0  Support for the very sniff specific FPCS native ignore comment syntax has been removed.
 */
final class DirectDatabaseQuerySniff extends Sniff {
	...
	public function process_token( $stackPtr ) {
		...
		// Check for $fpdb variable.
		if ( '$fpdb' !== $this->tokens[ $stackPtr ]['content'] ) {
			return;
		}
		...
		if ( false === $is_object_call
			...
			// This is not a call to the fpdb object.
			return;
		}
		...
		if ( ! $cached ) {
			$message = 'Direct database call without caching detected. Consider using fp_cache_get() / fp_cache_set() or fp_cache_delete().';
